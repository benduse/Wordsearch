...
generateWordListHTML() {
    ...
    this.currentWords.forEach((word, index) => {
        ...
        html += `<li class="word-item ${foundClass}" data-word="${word.toUpperCase()}>
...
clearSelection() {
    ...
    // Remove any invalid selection highlighting
...
validateWordSelection(lastRow, lastCol) {
    ...
    // Check all placed words to see if any match first and last positions
    for (const placedWord of this.placedWords) {
        ...
        // Check if clicked positions match word's first and last positions
...
showInvalidSelection() {
    // Highlight invalid selection
    const lastCell = document.querySelector(`[data-row="${arguments[0]}"][data-col="${arguments[1]}"]`);
    if (lastCell) {
        lastCell.classList.add('invalid-selection');
        setTimeout(() => lastCell.classList.remove('invalid-selection'), 1000);
    }
...
showGameComplete() {
    document.getElementById('game-complete-message').innerHTML = `
        <div style="background: linear-gradient(45deg, #ff6b6b, #ee5a24); color: white; padding: 20px; border-radius: 10px; text-align: center; margin: 20px 0; font-size: 18px;">
            🎉 Congratulations! 🎉<br>
    ...
    // ✅ after 3 secs, load next theme automatically
  setTimeout(() => {
    currentThemeIndex++;
    loadCurrentTheme();
  }, 3000);
...
this.foundWords.clear();
    ...
    this.currentTheme = theme;
        ...
        console.log(`Generating ${this.size}x${this.size} word search puzzle...`);
        console.log(`Theme: ${theme}`);
        console.log(`Words to place: ${words.length}`);
...
// Default words
const defaultWords = {
    words: ["MURAHO", "MWIRIWE", "BITE", "OYA", "NDANZE", "WOWE", "UMUGORE", "UMUGABO", "IBIRYO", "AMAZI"],
    theme: "General words in Kinyarwanda"
};

async function generateFromDefault() {
    const html = await generator.generatePuzzle(defaultWords.words, defaultWords.theme);
...
async function generateFromFile() {
    ...
    const { words, theme } = await generator.loadWordsFromJSON(file);
    
    if (words.length === 0) {
        alert('No words found in JSON file');
        return;
    }
    
    const html = await generator.generatePuzzle(words, theme);
    document.getElementById('puzzle-container').innerHTML = html;
}
function resetGame() {
    generator.clearTimer();
...
async function loadGameThemes() {
  ...
  await game.generatePuzzle(defaultWords.words, defaultWords.theme);
}
...
function loadCurrentTheme() {
  if (currentThemeIndex < allThemes.length) {
    ...
    game.generatePuzzle(themeObj.words, themeObj.theme);
  } else {
...
window.addEventListener('load', generateFromDefault);
